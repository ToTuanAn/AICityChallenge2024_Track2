...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        ###########################################################################################
        # Video-Llava
        if vision_tower is not None and video_tower is not None:
            assert vision_tower.hidden_size == video_tower.hidden_size
            assert vision_tower.num_patches == video_tower.num_patches
            self.config.mm_hidden_size = vision_tower.hidden_size
            num_patches = vision_tower.num_patches
        else:
            self.config.mm_hidden_size = max(getattr(vision_tower, "hidden_size", -1),
